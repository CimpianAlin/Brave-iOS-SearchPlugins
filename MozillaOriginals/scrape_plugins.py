...
def main():
    tmp_path = "tmp"
    search_plugins_path = "MozillaOriginals/SearchPlugins"
    ...
    if os.path.exists(search_plugins_path):
	    shutil.rmtree(search_plugins_path)
    shutil.move(tmp_path, search_plugins_path)
...
def downloadLocale(locale, scraper):
    ...
    directory = os.path.join("tmp", locale)
...
def verifyEngines():
    ...
    enDir = os.path.join("tmp", "en")
    for locale in os.listdir("tmp"):
...
def getSupportedLocales():
    supportedLocales = subprocess.Popen("./MozillaOriginals/get_supported_locales.swift", stdout=subprocess.PIPE).communicate()[0]
...
def saveDefault(locale, default):
    directory = os.path.join("tmp", locale, "default.txt")
...
class Scraper:
    ...
    def getFileList(self):
        ...
        if not response.ok:
            print "  ERROR: Could not read plugins directory"
            return None
    ...
    def getDefault(self):
        ...
        if not response.ok:
            print "  ERROR: Could not read prefs file"
            return None
        ...
        print "  ERROR: No default pref found"
        return None
